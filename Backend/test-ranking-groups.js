...
async function loginAsAdmin() {
  try {
    ...
    log.error(`خطأ في تسجيل الدخول كمسؤول: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function getGroups() {
  try {
    ...
    log.error(`خطأ في جلب الحلقات: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function getStudents() {
  try {
    ...
    if (response.data.success && response.data.data) {
      ...
      studentsGroup1 = allStudents.filter(s => s.group === group1Name).slice(0, 5);
      studentsGroup2 = allStudents.filter(s => s.group === group2Name).slice(0, 5);
      ...
      log.success(`تم جلب ${studentsGroup1.length} طلاب من ${group1Name} و ${studentsGroup2.length} طلاب من ${group2Name}`);
      return true;
    }
    ...
    log.error(`خطأ في جلب الطلاب: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function createTeacher1() {
  try {
    ...
    log.error(`خطأ في إنشاء المعلم الأول: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function createTeacher2() {
  try {
    ...
    log.error(`خطأ في إنشاء المعلم الثاني: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function loginAsTeacher1() {
  try {
    ...
    const teacherResponse = await axios.get(`${BASE_URL}/teachers/${teacherId}`, {
      headers: { Authorization: `Bearer ${adminToken}` }
    });
    ...
    log.error(`خطأ في تسجيل الدخول كمعلم أول: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function loginAsTeacher2() {
  try {
    ...
    // Get teacher data first
    const teacherResponse = await axios.get(`${BASE_URL}/teachers/${teacher2Id}`, {
      headers: { Authorization: `Bearer ${adminToken}` }
    });
    ...
    log.error(`خطأ في تسجيل الدخول كمعلم ثاني: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function test1_TeacherAddsOwnStudent() {
  try {
    ...
    log.error(`✗ فشل الاختبار 1: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function test2_TeacherAddsOtherGroupStudent() {
  try {
    ...
    try {
      ...
      log.error('✗ فشل الاختبار: يجب أن يُرفض طلب المعلم لإضافة طالب من حلقة أخرى');
      return false;
    } catch (error) {
      ...
      log.error(`✗ فشل الاختبار: خطأ غير متوقع - ${error.response?.data?.message || error.message}`);
      return false;
    }
  } catch (error) {
...
async function test3_TeacherAddsMultipleOwnStudents() {
  try {
    ...
    log.error(`✗ فشل الاختبار 3: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function test4_TeacherAddsMixedStudents() {
  try {
    log.info('اختبار 4: المعلم الأول يحاول إضافة مزيج من طلاب حلقته وحلقة أخرى...');
    ...
    try {
      ...
      log.error('✗ فشل الاختبار: يجب أن يُرفض طلب المعلم لإضافة مزيج من الطلاب');
      return false;
    } catch (error) {
      if (error.response && error.response.status === 403) {
...
async function test5_Teacher2AddsOwnStudents() {
  try {
    ...
    log.error('✗ فشل الاختبار: يجب أن ينجح المعلم الثاني في إضافة طالب من حلقته');
    return false;
  } catch (error) {
    log.error(`✗ فشل الاختبار 5: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
...
async function test6_AdminAddsAnyStudent() {
  try {
    ...
    log.error(`✗ فشل الاختبار 6: ${error.response?.data?.message || error.message}`);
    return false;
  }
}
