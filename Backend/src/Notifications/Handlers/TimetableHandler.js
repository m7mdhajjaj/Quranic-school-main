...
exports.notifyTimetableCreated = async (timetable, io) => {
  try {
    ...
    const notifications = students.map((student) => ({
      ...
      category: "academic",
      title: "موعد جديد",
      message: `موعد ${sessionTypeText} - ${dateStr} (${timetable.startHour})`,
      messageSummary: `${sessionTypeText} - ${dateStr}`,
      data: {
...
exports.notifyTimetableUpdated = async (timetable, changes, io) => {
  try {
    ...
    const notifications = students.map((student) => ({
      ...
      category: "academic",
      title: "تعديل موعد",
      message: `تم تعديل موعد ${dateStr}`,
      messageSummary: `تعديل - ${dateStr}`,
      data: {
        timetableId: timetable._id,
...
exports.notifyTimetableDeleted = async (timetable, io) => {
  try {
    ...
    const notifications = students.map((student) => ({
      ...
      category: "academic",
      title: "إلغاء موعد",
      message: `تم إلغاء موعد ${dateStr}`,
      messageSummary: `إلغاء - ${dateStr}`,
      data: {
        timetableId: timetable._id,
