// Backend/src/utils/Quran/QuranMetadata.js

/**
 * Quran Metadata Source of Truth
 * Contains Surah names, numbers, and ayah counts for validation.
 */
 ...
 * - Unixes Alef forms (أ, إ, آ -> ا)
 * - Unixes Ya forms (ى -> ي)
 * - Unixes Ta Marbuta (ة -> ه)
 * - Removes 'ال' (Al-Ta'rif) prefix optionally for comparison
 */
...
/**
 * Finds a Surah by name (flexible matching) or number.
 * @param {string|number} query - Surah name or number
 * @returns {object|null} Surah object { number, name, ayahCount } or null
 */
const findSurah = (query) => {
  if (!query) return null;

  // By Number
  if (!isNaN(query)) {
    const num = parseInt(query);
    return surahData.find((s) => s.number === num) || null;
  }

  // By Name
  const normalizedQuery = normalizeText(query);
  const normalizedQueryNoAl = normalizedQuery.startsWith("ال") 
    ? normalizedQuery.substring(2) 
    : normalizedQuery;

  // 1. Exact match (normalized)
  let found = surahData.find((s) => normalizeText(s.name) === normalizedQuery);
  if (found) return found;

  // 2. Partial match (contains) - e.g. "عمران" matches "آل عمران"
  found = surahData.find((s) => {
     const normName = normalizeText(s.name);
     return normName.includes(normalizedQuery) || normName.includes(normalizedQueryNoAl);
  });
...
/**
 * Get Surah by number directly
 */
const getSurahByNumber = (number) => {
  return surahData.find((s) => s.number === Number(number)) || null;
};
