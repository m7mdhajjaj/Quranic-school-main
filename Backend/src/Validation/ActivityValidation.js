...
const validateActivityTitle = (title) => {
  ...
  if (titleStr.length < 5) {
    return { isValid: false, message: 'عنوان النشاط يجب أن يكون 5 أحرف على الأقل' };
  }
  
  if (titleStr.length > 200) {
    return { isValid: false, message: 'عنوان النشاط يجب أن يكون 200 حرف أو أقل' };
  }
...
const validateDescription = (description) => {
  ...
  if (descStr.length < 10) {
    return { isValid: false, message: 'وصف النشاط يجب أن يكون 10 أحرف على الأقل' };
  }
  
  if (descStr.length > 2000) {
    return { isValid: false, message: 'وصف النشاط يجب أن يكون 2000 حرف أو أقل' };
  }
...
const validateActivityType = (type) => {
  ...
  const validTypes = [
    'عام', 'رياضي', 'ثقافي', 'تعليمي', 'فني', 'اجتماعي',
    'ديني', 'مسابقة', 'رحلة', 'ورشة عمل', 'محاضرة', 'معرض'
  ];
...
const validateActivityTime = (time) => {
  ...
  if (!/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(timeStr)) {
    return { isValid: false, message: 'وقت النشاط غير صحيح (يجب أن يكون بصيغة HH:MM)' };
  }
...
const validateDuration = (duration) => {
  ...
  if (isNaN(durationNum) || durationNum < 15) {
    return { isValid: false, message: 'مدة النشاط يجب أن تكون 15 دقيقة على الأقل' };
  }
  
  if (durationNum > 720) { // 12 hours max
...
const validateLocation = (location) => {
  ...
  if (locationStr.length < 2) {
    return { isValid: false, message: 'مكان النشاط يجب أن يكون حرفين على الأقل' };
  }
  
  if (locationStr.length > 200) {
    return { isValid: false, message: 'مكان النشاط يجب أن يكون 200 حرف أو أقل' };
  }
...
const validateMaxParticipants = (maxParticipants) => {
  ...
  if (isNaN(maxNum) || maxNum < 1) {
    return { isValid: false, message: 'العدد الأقصى للمشاركين يجب أن يكون رقم أكبر من صفر' };
  }
  
  if (maxNum > 1000) {
    return { isValid: false, message: 'العدد الأقصى للمشاركين لا يمكن أن يزيد عن 1000' };
  }
...
const validateActivityData = async (req, res, next) => {
  try {
    ...
    if (data.maxParticipants !== undefined) {
      const maxParticipantsValidation = validateMaxParticipants(data.maxParticipants);
