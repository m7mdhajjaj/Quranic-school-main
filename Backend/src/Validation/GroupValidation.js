...
const validateGroupName = (name) => {
  ...
  if (nameStr.length < 2) {
    return { isValid: false, message: 'اسم المجموعة يجب أن يكون حرفين على الأقل' };
  }
  
  if (nameStr.length > 100) {
    return { isValid: false, message: 'اسم المجموعة يجب أن يكون 100 حرف أو أقل' };
  }
  
  // Allow Arabic letters, numbers, spaces, and common punctuation
  if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\d\-'\.]+$/.test(nameStr)) {
    return { isValid: false, message: 'اسم المجموعة يجب أن يحتوي على أحرف عربية وأرقام فقط' };
  }
...
const validateDescription = (description) => {
  ...
  if (descStr.length > 500) {
    return { isValid: false, message: 'وصف المجموعة يجب أن يكون 500 حرف أو أقل' };
  }
...
const validateCapacity = (capacity) => {
  ...
  if (isNaN(capacityNum) || capacityNum < 1) {
    return { isValid: false, message: 'سعة المجموعة يجب أن تكون رقم أكبر من صفر' };
  }
  
  if (capacityNum > 100) {
    return { isValid: false, message: 'سعة المجموعة لا يمكن أن تزيد عن 100 طالب' };
  }
...
const validateSchedule = (schedule) => {
  ...
  for (const session of schedule) {
    ...
    if (!/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(session.time)) {
      return { isValid: false, message: `وقت غير صحيح: ${session.time} (يجب أن يكون بصيغة HH:MM)` };
    }
