...
exports.updateSection = async (req, res) => {
  try {
    ...
    if (targetGroup) {
      ...
      if (
        updateData.date &&
        new Date(updateData.date).getTime() !== new Date(section.date).getTime()
      ) {
        const currentWeekCheck =
          sequenceService.checkCurrentWeekOnly(targetDate);
      ...
      if (targetGroupId) {
        ...
        if (
          ...
          updateData.memorizationMeta.length > 0
        ) {
      ...
      const memMetaToCheck =
        updateData.memorizationMeta || section.memorizationMeta;
