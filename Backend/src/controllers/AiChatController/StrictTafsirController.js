...
const { fuzzyFindSurah, findClosestSurahs } = require("../../utils/textAnalysis");
...
async function extractVerseKey(question) {
  ...
  // 3️⃣ البحث الدقيق باسم السورة
  // نستخدم SURAH_CANONICAL المستوردة بدلاً من القاموس الضخم المكرر
  let foundSurah = null;
  let foundAyah = null;
    ...
    for (const name of surah.names) {
      ...
      if (normalizedName.length <= 2) {
         ...
         if (regex.test(normalizedText)) {
           ...
           console.log(`📌 Surah name detected (short): ${name} = ${surah.number}`);
           break;
         }
      } else {
        if (normalizedText.includes(normalizedName)) {
          foundSurah = surah.number;
          console.log(`📌 Surah name detected (exact): ${name} = ${surah.number}`);
          break;
        }
      }
    }
  ...
  const hasTafsirKeyword = /تفسير|فسر|معن[ىي]|شرح|سور[ةه]|آي[ةه]|اي[ةه]/.test(text);
...
function classifyQuestion(question) {
  ...
  // 🚫 0️⃣ منع اللغة الإنجليزية والأحرف اللاتينية تماماً
  if (/[a-zA-Z]/.test(text)) {
  ...
  // 1️⃣ أنماط الدردشة العامة (التحيات فقط - العربية)
  const casualPatterns = [
    /^(مرحبا|اهلا|هلا|السلام عليكم|صباح الخير|مساء الخير|هاي|هي)/i,
    /^(كيف حالك|كيفك|شو اخبارك|شلونك|ازيك|عامل ايه)/i,
    /^(شكرا|مشكور|الله يعطيك العافيه|يسلمو)/i,
    /^(من انت|مين انت|ما اسمك|شو اسمك|عرفني عن نفسك)/i,
    /^(ماذا تفعل|شو بتعمل|ايش تسوي)/i,
    /^(كيف استخدمك|كيف اسالك|كيف بشتغل معك)/i,
    /^(مع السلامه|باي|الى اللقاء|وداعا)/i,
    /^(اوك|تمام|حسنا|نعم|لا)$/i,
  ];

  for (const pattern of casualPatterns) {
    if (pattern.test(text)) { // استخدام النص الأصلي للتحيات الإنجليزية
      return { type: "casual", reason: "greeting_or_casual" };
    }
    if (pattern.test(normalizedText)) { // استخدام النص المطبع
      return { type: "casual", reason: "greeting_or_casual" };
    }
  }
  ...
  const forbiddenPatterns = [
    ...
    /رأي/, /حكم/, /فتو[ىي]/, /هل يجوز/, /حلال/, /حرام/, /قارن/, /الفرق بين/,
    /ما\s*حكم/, /ماحكم/, /هل\s*يصح/, /هل\s*تصح/, /مباح/, /مكروه/, /واجب/, /سنة/, /بدعة/,
    /كفارة/, /فدية/, /زكاة/, /صلاة/, /صلاه/, /وضوء/, /صيام/, /حج/, /عمرة/, /طهارة/,
    /طلاق/, /زواج/, /ميراث/, /عده/, /نفاس/, /حيض/, /جنابة/, /استحاضة/,

    // تجويد
    /تجويد/, /مخارج/, /صفات/, /إدغام/, /ادغام/, /إخفاء/, /اخفاء/, /إقلاب/, /اقلاب/, 
    /إظهار/, /اظهار/, /مد/, /غن[ةه]/, /قلقلة/, /ترقيق/, /تفخيم/,
    // مواضيع أخرى
    /حديث/, /سيرة/, /تاريخ/, /فقه/, /عقيدة/,
  ];
  ...
  const tafsirPatterns = [
    /تفسير/, /فسر/, /معن[ىي]/, /شرح/, /ما معنى/, /ما المقصود/, /المقصود ب/,
  ];
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت "المساعد الإسلامي" - متخصص في تفسير القرآن الكريم فقط.

✅ مهامك المسموحة:
1. الرد الودود على التحيات (السلام عليكم، كيف حالك، شكراً).
2. الترحيب بالمستخدم وشرح دورك (أنا بوت لتفسير القرآن من تفسير ابن كثير).

❌ تحذير صارم:
- إذا خرج السؤال عن التحيات أو التفسير، اعتذر وقل "أنا غير مصرح لي بالإجابة".
- ممنوع التجويد، الفتاوى، الأحاديث، أو أي موضوع عام.
...
exports.chat = async (req, res) => {
  ...
  if (!message || message.trim().length < 2) {
    return res.status(400).json({ success: false, message: "يرجى إدخال سؤال صحيح" });
  }
  ...
  try {
    ...
    console.log(`📊 Validating Question: ${classification.type} (${classification.reason})`);
    ...
    if (classification.type === "casual") {
      ...
      return res.json({
        ...
        data: {
          ...
          timestamp: new Date().toISOString(),
          metadata: { ...requestLog, processingTime: `${Date.now() - startTime}ms` }
        }
      });
    }

    // حالة: ممنوع (تجويد، فتاوى، أو غير معروف)
    ...
    if (verseKey.surah && !verseKey.ayah) {
      ...
      const surahInfo = SURAH_CANONICAL.find(s => s.number === verseKey.surah);
      const surahName = surahInfo ? surahInfo.names[0] : `رقم ${verseKey.surah}`;
      ...
      return res.json({
        ...
        data: {
          ...
          timestamp: new Date().toISOString(),
          metadata: { ...requestLog, surah: verseKey.surah, waitingForAyah: true },
        },
      });
    }
...
exports.speak = async (req, res) => {
  const { text } = req.body;
  if (!text || text.length < 5) return res.status(400).json({ success: false, message: "النص قصير جداً" });
  ...
  try {
    ...
    if (inputText.length > 1000) {
       ...
       inputText = lastPeriod > 0 ? truncated.substring(0, lastPeriod + 1) : truncated;
    }
...
exports.transcribe = async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, message: "يرجى إرسال ملف صوتي" });
