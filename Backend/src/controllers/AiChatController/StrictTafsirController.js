...
async function extractVerseKey(question) {
  ...
  if (!foundSurah) {
    ...
    if (fuzzyResult && fuzzyResult.confidence >= 0.6) {
      ...
      console.log(
        `🔮 Fuzzy match: "${text}" → ${fuzzyResult.surahName} (${(fuzzyResult.confidence * 100).toFixed(0)}% confidence)`,
      );
    }
  }
  ...
  if (text.length > 3) {
    ...
    if (aiResult.surah && aiResult.surah >= 1 && aiResult.surah <= 114) {
      console.log(
        `🤖 AI corrected: "${text}" → "${aiResult.corrected}" (Surah ${aiResult.surah}, Ayah ${aiResult.ayah})`,
      );
