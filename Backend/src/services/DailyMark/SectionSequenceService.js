...
class SectionSequenceService {
  ...
  async getLastProgress(
    ...
    const metaField =
      type === "memorization" ? "memorizationMeta" : "reviewMeta";
  ...
  async getMaxProgress(
    ...
    for (const section of sections) {
      ...
      const segments = section[metaField].filter(
  ...
  async validateSequence(
    ...
    for (const seg of newSegments) {
      ...
      const potentialConflicts = await Section.find(overlapQuery).select(
        `date dateKey ${metaField}`,
      );

      for (const conflictingSection of potentialConflicts) {
        // أ. تكرار الحفظ (ممنوع منعاً باتاً - في أي تاريخ)
        if (type === "memorization") {
          const conflictSeg = conflictingSection[metaField].find(
            (s) =>
          ...
          if (conflictSeg) {
            ...
            return {
              isValid: false,
              message: MEMORIZATION_ERRORS.DUPLICATE(
                seg.surahNumber,
                seg.ayahStart,
                seg.ayahEnd,
                dateStr,
        ...
        if (type === "review" && newDateKey) {
          const conflictDateKey =
            conflictingSection.dateKey ||
            this.toDateKeyLocal(conflictingSection.date);

          if (conflictDateKey === newDateKey) {
            const exactMatch = conflictingSection[metaField].find(
              (s) =>
                s.surahNumber === seg.surahNumber &&
            ...
            if (exactMatch) {
              return {
                isValid: false,
                message: REVIEW_ERRORS.SAME_DAY_DUPLICATE(
                  seg.surahNumber,
                  seg.ayahStart,
                  seg.ayahEnd,
                ),
      ...
      if (type === "review") {
        ...
        const maxMemProgress = await this.getMaxProgress(
          groupId,
          seg.surahNumber,
          "memorization",
          null,
        ...
        if (siblingSegments && siblingSegments.length > 0) {
          const siblingOfSameSurah = siblingSegments.filter(
            (m) => m.surahNumber === seg.surahNumber,
          );
        ...
        // ✅ تم تعطيل فحص الحفظ السابق للمراجعة - يمكن إضافة أي سورة للمراجعة بدون التحقق
        // (الفحوصات القديمة معلقة)
      }
      ...
      if (type === "memorization") {
        ...
        const hasLocalPredecessor = newSegments.some(
          (s) =>
            s !== seg &&
            s.surahNumber === seg.surahNumber &&
        ...
        const hasLocalSuccessor = newSegments.some(
          (s) =>
            s !== seg &&
            s.surahNumber === seg.surahNumber &&
        ...
        const effectivePrevious = hasLocalPredecessor
          ? null
          : neighbors.previous;
        ...
        if (!validationResult.isValid) {
          ...
          if (!validationResult.message.startsWith("❌")) {
            validationResult.message = MEMORIZATION_ERRORS.SEQUENCE_ERROR(
        ...
        if (
          ...
          return {
            isValid: false,
            message: MEMORIZATION_ERRORS.WRONG_START(
              seg.surahNumber,
              seg.ayahStart,
            ),
          };
  ...
  async getMatchingMemorizationEnd(groupId, surahNumber, ayahStart) {
    ...
    const segment = section.memorizationMeta.find(
      (s) => s.surahNumber === surahNumber && s.ayahStart === ayahStart,
    );
  ...
  validateConsistency(memorizationMeta, reviewMeta) {
    ...
    for (const rev of reviewMeta) {
      ...
      const memSegments = memorizationMeta.filter(
      ...
      for (const mem of memSegments) {
        ...
        if (rev.ayahStart === mem.ayahStart && rev.ayahEnd === mem.ayahEnd) {
          return {
            isValid: false,
            message: CONSISTENCY_ERRORS.SAME_SEGMENT(
              rev.ayahStart,
              rev.ayahEnd,
              rev.surahNumber,
            ),
        ...
        if (rev.ayahEnd >= mem.ayahStart) {
          return {
            isValid: false,
            message: CONSISTENCY_ERRORS.REVIEW_OVERLAPS_MEMORIZATION(
              rev.ayahStart,
              rev.ayahEnd,
              mem.ayahStart,
              mem.ayahEnd,
   ...
   * @returns {{ isValid: boolean, message?: string }}
   */
  validateInsertionWithNeighbors(
    newSegment,
    previousNeighbor,
    nextNeighbor,
    type,
    ...
    if (previousNeighbor) {
      ...
      if (type === "memorization") {
        ...
        if (newSegment.ayahStart !== expectedStart) {
          return {
            isValid: false,
            message: MEMORIZATION_ERRORS.GAP(
              expectedStart,
    ...
    if (nextNeighbor) {
      ...
      if (type === "memorization") {
        ...
        if (newSegment.ayahEnd !== expectedEnd) {
          const nextDate = nextNeighbor.dateKey
            ? new Date(nextNeighbor.date).toLocaleDateString("ar-EG", {
                year: "numeric",
                month: "long",
                day: "numeric",
                timeZone: TIMEZONE,
          ...
          return {
            isValid: false,
            message: MEMORIZATION_ERRORS.NEXT_GAP(
              newSegment.ayahEnd,
              nextNeighbor.ayahStart,
              nextDate,
            ),
          };
        }
      ...
      if (newSegment.ayahEnd >= nextNeighbor.ayahStart) {
        ...
        return {
          isValid: false,
          message: MEMORIZATION_ERRORS.NEXT_OVERLAP(
            newSegment.ayahEnd,
            nextNeighbor.ayahStart,
            nextDate,
          ),
    ...
    if (type === "memorization" && previousNeighbor && nextNeighbor) {
      ...
      if (newSegment.ayahStart !== gapStart || newSegment.ayahEnd !== gapEnd) {
        return {
          isValid: false,
          message: MEMORIZATION_ERRORS.BRIDGE_INCOMPLETE(
            segDesc,
            gapStart,
            gapEnd,
            previousNeighbor.canonicalKey,
            nextNeighbor.canonicalKey,
          ),
        };
      }
    }
  ...
  async validateMonotonicOrder(
    ...
    for (const section of sections) {
      const metas = section[metaField].filter(
    ...
    existingSegments.forEach((seg, i) => {
      console.log(
        `   ${i + 1}. Date: ${seg.dateKey} | Ayahs: ${seg.ayahStart}-${seg.ayahEnd} | SectionId: ${seg.sectionId}`,
      );
    });
    ...
    for (const existing of existingSegments) {
      ...
      console.log(
        `   proposedTime < existingTime: ${proposedTime < existingTime}`,
      );
      console.log(
        `   proposedTime > existingTime: ${proposedTime > existingTime}`,
      );
      console.log(
        `   proposedTime === existingTime: ${proposedTime.getTime() === existingTime.getTime()}`,
      );
      ...
      if (proposedTime < existingTime) {
        ...
        console.log(
          `   Check: ayahStart(${ayahStart}) > existing.ayahStart(${existing.ayahStart}) = ${ayahStart > existing.ayahStart}`,
        );
        // يجب أن تكون الآيات المقترحة أقدم أو مساوية
        if (ayahStart > existing.ayahStart) {
          console.log(
            "   ❌ CONFLICT DETECTED - Proposed ayahs are NEWER but date is OLDER",
          );
          console.log(
            "========== validateMonotonicOrder END (INVALID) ==========\n",
          );
      ...
      if (proposedTime > existingTime) {
        ...
        if (ayahStart < existing.ayahStart) {
          console.log(
            "   ❌ CONFLICT DETECTED - Proposed ayahs are OLDER but date is NEWER",
          );
          console.log(
            "========== validateMonotonicOrder END (INVALID) ==========\n",
          );
