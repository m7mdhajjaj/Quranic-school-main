    ...
    this.autoPresenceJob = cron.schedule(
      ...
      async () => {
        ...
        try {
          ...
          // 2. جلب المقاطع (Sections) التي كانت بالأمس
          ...
          if (sections.length === 0) {
            console.log("ℹ️ [AutoPresence] لم يتم العثور على مقاطع (Sections) ليوم أمس. تجاوز فحص الحضور.");
            return;
          }
          ...
          for (const groupName of groupsWithSections) {
            ...
            const missingStudents = students.filter(s => !presentStudentIds.has(s._id.toString()));
