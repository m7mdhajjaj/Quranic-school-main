 ...
 */
const minDateInFuture = yup.date().test(
  'min-date-future',
...
export const examScheduleSchema = yup.object().shape({
  ...
  type: yup
    .string()
    .oneOf(['شفهي', 'كتابي', 'تقييم شامل'], 'نوع الامتحان يجب أن يكون: شفهي، كتابي، أو تقييم شامل')
    .default('شفهي'),
  ...
  time: yup
    .string()
    .matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'صيغة الوقت غير صحيحة (يجب أن تكون HH:MM)')
    .test(
      'time-range',
  ...
  // Total marks validation (10-40)
  totalMarks: yup
    .number()
    .min(10, 'مجموع الدرجات يجب أن يكون 10 على الأقل')
    .max(40, 'مجموع الدرجات لا يمكن أن يزيد عن 40')
    .default(20)
    .nullable(),
 ...
 */
export const validateExamData = async (data: any): Promise<{ isValid: boolean; errors: string[] }> => {
  try {
 ...
 */
export const validateField = async (fieldName: string, value: any): Promise<{ isValid: boolean; error?: string }> => {
  try {
