...
export const useAzkarCategories = (categories: AzkarCategory[]) => {
  ...
  /**
   * تحويل التصنيفات لتتضمن الإحصائيات
   */
  ...
  /**
   * الحصول على إجمالي الإحصائيات لجميع التصنيفات
   */
  const totalStats = useMemo(() => {
    ...
    const allCompleted = categoriesWithStats.every((cat) => cat.isFullyCompleted);

    return {
      totalCompleted,
      totalAll,
      progress: totalAll > 0 ? Math.round((totalCompleted / totalAll) * 100) : 0,
      allCompleted,
    };
  }, [categoriesWithStats]);
