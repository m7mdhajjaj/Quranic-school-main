...
export const LoginCard: React.FC<LoginCardProps> = ({ children, error, success }) => {
  ...
  useEffect(() => {
    ...
    if (typeof (window as any).lastError === "undefined") (window as any).lastError = "";
    if (typeof (window as any).lastSuccess === "undefined") (window as any).lastSuccess = false;
    if (typeof error !== "undefined" && error && error !== (window as any).lastError) {
      setShake(true);
      playError();
      (window as any).lastError = error;
      setTimeout(() => setShake(false), 600);
    }
    if (typeof success !== "undefined" && success && success !== (window as any).lastSuccess) {
      setShowSuccess(true);
      playSuccess();
      (window as any).lastSuccess = success;
      setTimeout(() => setShowSuccess(false), 1200);
    }
  }, [error, success, playError, playSuccess]);
