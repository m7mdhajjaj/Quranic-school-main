...
const Arrangement = () => {
  ...
  const handleScoreChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate score is a valid number and within range
  ...
  const saveRanking = async () => {
    ...
    // Validate that student is from the correct group
    const selectedStudentData = filteredStudents.find(
      (s) => s._id === newRankingEntry._id
    );
    if (!selectedStudentData) {
      alert("الطالب المحدد غير موجود في الحلقة الحالية");
      return;
    }
    ...
    try {
      ...
      // Only include students from the same group
        ...
        ranking?.topThree
          .filter(
            (item) =>
              ...
              item.studentId &&
              item.studentId._id &&
              item.studentId.group === selectedStudentData.group
          )
          .map((item) => ({
      ...
      // Ensure we have at least one student in topTen
      if (topTen.length === 0) {
        alert("لا يوجد طلاب للإضافة في الترتيب");
        return;
      }
      ...
      console.log("Sending ranking data:", rankingData);

      await createRanking(rankingData);

      // Refresh ranking data with proper group
      ...
      try {
        ...
        console.log("Could not refresh ranking, but save was successful");
      }
      ...
      try {
        ...
        console.log("Could not refresh periods, but save was successful");
      }
      ...
      if (axiosError?.response?.data?.message) {
        ...
        errorMessage =
          "البيانات المدخلة غير صحيحة. تأكد من أن جميع الطلاب من نفس الحلقة";
      } else if (axiosError?.response?.status === 403) {
        errorMessage = "ليس لديك صلاحية لإضافة هؤلاء الطلاب";
      }
