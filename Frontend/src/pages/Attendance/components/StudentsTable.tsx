...
export const StudentsTable = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      {/* 🆕 رسالة عدم وجود مقطع */}
      {noSectionInfo?.noSection ? (
        <div className="text-center py-12">
          <div className="w-16 h-16 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <Calendar className="w-8 h-8 text-amber-500" />
          </div>
          <h3 className="text-lg font-medium text-gray-900">لا يوجد مقطع في هذا التاريخ</h3>
          <p className="text-gray-500 mt-1">{noSectionInfo.message}</p>
          <p className="text-amber-600 mt-2 text-sm">💡 قم بإضافة مقطع في سجل المتابعة اليومي أولاً</p>
        </div>
      ) : students.length === 0 ? (
        <div className="text-center py-12">
