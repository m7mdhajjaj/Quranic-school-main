...
const MessageItem: React.FC<MessageItemProps> = React.memo(
  ({ message, isOwn, onReply, onReplyClick, onDelete, onEdit }) => {
    const {
      isEditing,
      editText,
      setEditText,
    ...
    const getDropdownItems = () => {
      ...
      if (isOwn && !message.deletedForAll) {
        ...
        const isRead =
          message.readAt || (message.seenBy && message.seenBy.length > 0);
              ...
              content={
                <div className="max-h-40 overflow-y-auto custom-scrollbar p-1">
                  {message.seenBy.map((seen: SeenByItem) => (
                    <div
                      key={seen.userId}
                      className="py-1 border-b border-gray-700/50 last:border-0 text-xs whitespace-nowrap">
                      تمت المشاهدة بواسطة {seen.user?.firstName} في{" "}
                      {new Date(seen.seenAt).toLocaleTimeString("ar", {
                        hour: "2-digit",
                        minute: "2-digit",
    ...
    return (
      <div
        ...
        <div
          className={`max-w-[75%] ${isOwn ? "order-first" : ""} relative group`}>
          {/* Action Buttons - Compact Column Layout */}
          <div
            className={`absolute -top-1 ${isOwn ? "-left-9" : "-right-9"} flex flex-col items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-200 z-20`}>
            {/* Dropdown Menu */}
            <DropdownMenu
            ...
            {isEditing ? (
              ...
              <>
                ...
                <p className="whitespace-pre-wrap break-words leading-relaxed text-[15px]">
                  {message.text
                    .split(/(@[\u0600-\u06FFa-zA-Z0-9\s]+)/g)
                    .map((part: string, i: number) => {
                      // Ensure mentions is an array before calling .some()
                      const mentionsArray = Array.isArray(message.mentions)
                        ? message.mentions
                        : [];
                      const isMention = mentionsArray.some((m: MentionItem) => {
                        if (m.type === "all" && part.trim() === "@الجميع")
                          return true;
                        if (
                          m.type === "user" &&
                          m.user &&
                      ...
                      if (isMention || part.trim() === "@الجميع") {
                        return (
                          <span
                            key={i}
                            className={`font-bold px-1 rounded ${isOwn ? "bg-white/20" : "bg-emerald-100 text-emerald-700"}`}>
                            {part}
                          </span>
                        );
                      }
                ...
                <div
                  className={`flex items-center gap-1 mt-1 select-none ${isOwn ? "justify-end text-emerald-100" : "justify-end text-gray-400"}`}>
                  {message.edited && (
