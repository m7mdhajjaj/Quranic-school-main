...
const MarksManagement: React.FC<MarksManagementProps> = () => {
  ...
  const fetchStudentsAndMarks = async () => {
    ...
    try {
      ...
      const selectedGroupData = groupsData.find(g => g.name === selectedExam.group);
      ...
      const studentsWithGroup = selectedGroupData.students.map(s => ({
        ...s,
      ...
      const formattedMarks = Array.isArray(marksData) ? marksData.map((m: any) => ({
        _id: m._id,
        studentId: typeof m.student === 'string' ? m.student : m.student._id,
  ...
  const handleUpdateMark = async (markId: string, newMark: number) => {
    if (!selectedExam || newMark < 0 || newMark > (selectedExam.totalMarks || 100)) {
      showErrorToast(`العلامة يجب أن تكون بين 0 و ${selectedExam?.totalMarks || 100}`);
      return;
    }

    // تحديث فوري للواجهة (Optimistic Update)
    const previousMarks = [...marks];
    setMarks(marks.map(m => m._id === markId ? { ...m, mark: newMark } : m));
    setEditingMarkId(null);

    try {
      await api.put(
        `/exam-schedule/marks/${markId}`,
        { mark: newMark }
      );
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await api.post(
        `/exam-schedule/marks/bulk-delete`,
        {
          examId: selectedExam._id,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl shadow-lg p-6">
        ...
        <p className="text-emerald-50 mt-2">إدخال وتعديل وحذف علامات الطلاب لكل حلقة</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-gray-500 text-lg font-medium">لا توجد حلقات متاحة</p>
              <p className="text-gray-400 text-sm mt-2">يرجى التأكد من أن لديك حلقات مسجلة</p>
            </div>
          ) : (
            groupsData.map((groupCard) => (
              <div
                ...
                className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:-translate-y-1 overflow-hidden group" dir="rtl"
              >
                <div className="bg-gradient-to-l from-emerald-500 to-teal-600 p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-white">{groupCard.name}</h3>
                        <p className="text-emerald-50 text-sm">حلقة قرآنية</p>
                      </div>
                    </div>
                    <ArrowRight className="w-6 h-6 text-white group-hover:-translate-x-1 transition-transform" style={{ transform: 'rotate(180deg)' }} />
                  </div>
                </div>
                <div className="p-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-emerald-600">{groupCard.totalStudents}</p>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-blue-600">{groupCard.examCount}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {exams.map((exam) => (
                <div
                  ...
                  className="border-2 border-gray-200 rounded-xl p-4 hover:border-emerald-500 hover:shadow-lg transition-all cursor-pointer group" dir="rtl"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <ArrowRight className="w-5 h-5 text-gray-400 group-hover:text-emerald-600 group-hover:-translate-x-1 transition-all" style={{ transform: 'rotate(180deg)' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200 space-y-4">
            ...
            {selectedStudents.size > 0 && (
              <div className="flex items-center justify-between bg-emerald-50 p-3 rounded-lg" dir="rtl">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-emerald-50 border-b-2 border-emerald-200">
                  ...
                  <th className="px-6 py-4 text-center text-sm font-bold text-gray-700">الحلقة</th>
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-gray-100 hover:bg-emerald-50/50 transition-colors ${
                          selectedStudents.has(student._id) ? 'bg-emerald-100/50' :
                          ...
                          {studentMark ? (
                            isEditing ? (
                              ...
                              <span className={`inline-flex items-center px-3 py-1 rounded-lg font-bold text-sm ${
                                studentMark.mark >= (selectedExam.totalMarks || 100) * 0.5
                            ...
                            <span className="text-gray-400 text-sm">لم يتم الإدخال</span>
                          )}
                        </td>
                          ...
                          <div className="flex items-center justify-center gap-2">
                            {studentMark ? (
                              isEditing ? (
                                <>
                                  <button
                                    onClick={() => handleUpdateMark(studentMark._id!, editValue)}
                                ...
                                <>
                                  ...
                                  <button
                                    onClick={() => handleDeleteMark(studentMark._id!)}
                              ...
                              <button
                                onClick={() => handleAddMark(student._id, student.name)}
