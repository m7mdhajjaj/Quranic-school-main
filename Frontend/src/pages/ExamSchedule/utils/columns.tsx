...
const formatDateArabic = formatArabicDate;
...
const AvgBadge: React.FC<{ value?: number | null }> = ({ value }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-3 py-1 bg-${color}-100 text-${color}-700 rounded-full text-sm font-semibold`}>
      {value.toFixed(1)}%
    </span>
  );
};

const StudentMarkDisplay: React.FC<{ mark?: string; isDesktop?: boolean }> = ({
  mark,
}) => {
  if (!mark) {
  ...
  const cols: Column<Exam>[] = [
    {
      ...
      render: (exam: Exam) => (
        <div className="flex flex-col gap-1">
          <span className="font-semibold text-emerald-900 text-sm break-words">
            {exam.name}
          </span>
          {exam.subject && (
    ...
    },
    {
      key: "result",
      header:
        role === "teacher" || role === "admin" ? "متوسط العلامات" : "النتيجة",
