...
const Timetable = () => {
  // 🔌 Socket Connection
  const { isConnected: socketConnected, lastUpdate: socketLastUpdate, socketId } = useTimetableSocket();
  const days = [
    "السبت",
    "الأحد",
    "الاثنين",
  ...
  useEffect(() => {
    if (socketLastUpdate) {
      console.log('🔄 Timetable Socket update received, refreshing sessions...');
      fetchSessions();
    }
  }, [socketLastUpdate, fetchSessions]);
