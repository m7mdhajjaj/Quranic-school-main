...
const Profile: React.FC = () => {
  ...
  // ------------------------
  // Load user on mount (axios + smart 404 fallback)
  // ------------------------
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const stored = JSON.parse(userJson);
        const userId: string = stored._id;
        const axios = makeAxios(token);

        // Decide which endpoint to try first
        // Strongest hints first
        const loginAs: EffectiveRole | undefined = stored?.loginAs;
        let first: Endpoint = 'students';
        if (loginAs === 'teacher' || loginAs === 'admin') first = 'teachers';
        else if (stored?.role === 'teacher' || stored?.role === 'admin') first = 'teachers';
        else if (stored?.teacherId) first = 'teachers';
        else if (stored?.studentId) first = 'students';

        const order: Endpoint[] = first === 'teachers' ? ['teachers', 'students'] : ['students', 'teachers'];
        ...
        for (const ep of order) {
          try {
            const resp = await axios.get(`/${ep}/${userId}`);
            // accept both {data: {...}} and raw object
            const body = resp.data && typeof resp.data === 'object' ? resp.data : {};
            const data = body?.data ?? body;
            if (!data || !data._id) throw new Error('Unexpected response shape');
            fetched = data;
            usedEndpoint = ep;
            break;
          } catch (e: any) {
            ...
            // Only fall back when it's truly a 404 Not Found
            if (status === 404) {
              continue;
            } else {
              // Any other error -> stop and show
              throw e;
            }
          }
        }

        if (!fetched || !usedEndpoint) {
          console.error('Failed to fetch user from both endpoints:', lastErr);
          toast.error('تعذّر تحميل البيانات (لم يتم العثور على المستخدم في المسارات المتاحة)');
          return;
        }

        // Derive effective role from winning endpoint if not present
        let effectiveRole: EffectiveRole =
          (stored?.role as EffectiveRole) ||
          (fetched?.role as EffectiveRole) ||
          (usedEndpoint === 'teachers' ? 'teacher' : 'student');

        // If loginAs explicitly admin, respect it
        if (loginAs === 'admin') effectiveRole = 'admin';

        const finalUser = { ...fetched, role: effectiveRole } as User;
        setUser(finalUser);
        setEditedUser(finalUser);

        // persist merged role into localStorage to avoid future wrong calls
        try {
          const mergedStored = { ...stored, role: effectiveRole };
          localStorage.setItem('user', JSON.stringify(mergedStored));
        } catch {}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 1) avatar upload (if backend supports it)
      if (avatarFile) {
        const fd = new FormData();
        fd.append('avatar', avatarFile);
        const up = await axios.post(`/${endpoint}/${user._id}/avatar`, fd, {
          headers: { Authorization: `Bearer ${token}` }, // axios sets content-type automatically for FormData
        });
        const body = up.data && typeof up.data === 'object' ? up.data : {};
        const newAvatar = (body?.data?.avatar ?? body?.avatar) as string | undefined;
        if (newAvatar) {
          setEditedUser((prev) => (prev ? { ...prev, avatar: newAvatar } : prev));
          setAvatarPreview(
            newAvatar.startsWith('http') ? newAvatar : `${API_URL}/uploads/${newAvatar}`
          );
        }
      }
      ...
      const resp = await axios.put(`/${endpoint}/${user._id}`, payload);
      const body = resp.data && typeof resp.data === 'object' ? resp.data : {};
      const updated: User = (body?.data ?? body) as User;
      ...
      const msg =
        ...
        'فشل في تحديث البيانات. حاول مجددًا.';
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const handleCancel = () => {
    ...
    setIsEditing(false);
    setAvatarFile(null);
    setAvatarPreview(user?.avatar ? (user.avatar.startsWith('http') ? user.avatar : `${API_URL}/uploads/${user.avatar}`) : '');
  };
              ...
              <div className="bg-slate-50 rounded-xl p-4 flex flex-col items-center">
                ...
                {isEditing ? (
                  <>
                    ...
                    {!canEditBirthDate() && (
                      <span className="text-xs text-red-500 mb-2">
                        لا يمكنك تعديل تاريخ الميلاد إلا مرتين في الشهر، ستتجدد المحاولات في بداية الشهر القادم
                      </span>
                    )}
                    <div className="w-full flex flex-row items-center justify-between gap-2 mb-2">
                      <span className="text-xs text-gray-500">العمر:</span>
                  ...
                  <div className="w-full flex flex-col items-center">
                    ...
                    <span className="text-sm text-emerald-700 font-bold mb-1">
                      العمر: {calculateAge(displayUser.birthDate)} سنة
                    </span>
                    <span className="text-sm text-slate-600">
                      {displayUser.gender ? `الجنس: ${displayUser.gender}` : ''}
                    </span>
                  </div>
                )}
              </div>
              ...
              {isStudent && (
                <div className="bg-slate-50 rounded-xl p-4 flex flex-col items-center">
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-sm text-slate-600">
                      {displayUser.group || displayUser.groups?.join(', ') || 'غير محدد'}
                    </p>
                  )}
                </div>
              )}
