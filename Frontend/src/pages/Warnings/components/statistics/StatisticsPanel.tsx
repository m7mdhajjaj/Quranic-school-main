...
import { getWarningLabel, formatArabicDate } from '../../types/Constans';
import {
  BarChart3,
  Users,
  AlertTriangle,
...
const WARNING_ICONS = {
  ...
  first: <ShieldAlert className="w-4 h-4" />,
  second: <AlertOctagon className="w-4 h-4" />,
  third: <Ban className="w-4 h-4" />,
  expulsion: <XCircle className="w-4 h-4" />,
} as const;

export const StatisticsPanel: React.FC<StatisticsPanelProps> = React.memo(
  ({ statistics }) => {
    // ✅ Memoize checks
    const hasRecentWarnings = useMemo(
      () => statistics.recentWarnings && statistics.recentWarnings.length > 0,
      [statistics.recentWarnings]
    );

    const hasTopReasons = useMemo(
      () => statistics.topReasons && statistics.topReasons.length > 0,
      [statistics.topReasons]
    );

    const hasWarningsByGroup = useMemo(
      () => statistics.warningsByGroup && statistics.warningsByGroup.length > 0,
      [statistics.warningsByGroup]
    );

    return (
      <Card
        className="mb-8 bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 border-2 border-blue-200/50 shadow-2xl backdrop-blur-sm relative overflow-hidden"
        padding="lg"
      >
        {/* Animated Background */}
        ...
        <div className="relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <Card
              variant="elevated"
              padding="md"
              className="bg-gradient-to-br from-white to-blue-50/50 border border-blue-100 hover:shadow-xl transition-shadow duration-300"
            >
              <h3 className="text-lg font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent mb-3">
                الإنذارات حسب النوع
              </h3>
              <div className="space-y-2">
                {Object.entries(statistics.warningsCount).map(
                  ([type, count]) => {
                    const icon =
            ...
            <Card
              variant="elevated"
              padding="md"
              className="bg-gradient-to-br from-white to-purple-50/50 border border-purple-100 hover:shadow-xl transition-shadow duration-300"
            >
              <h3 className="text-lg font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent mb-3">
                أكثر الأسباب تكراراً
              </h3>
            ...
            <div className="space-y-2">
              {hasRecentWarnings ? (
                statistics.recentWarnings.map((warning) => {
                  const icon =
                    WARNING_ICONS[warning.type as keyof typeof WARNING_ICONS];
                  return (
                    <Card
                      key={warning._id}
                      variant="default"
