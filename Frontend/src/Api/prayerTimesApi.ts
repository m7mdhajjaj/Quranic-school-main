...
export const getPrayerTimes = async (latitude?: number, longitude?: number): Promise<PrayerTimesResponse> => {
  try {
    ...
    // Fallback to external API if backend not available
...
export const getQiblaDirection = async (latitude?: number, longitude?: number): Promise<QiblaResponse> => {
  try {
    // Try backend first
...
export const savePrayerSettings = async (settings: PrayerSettings): Promise<{ success: boolean; message?: string }> => {
  const response = await api.post('/prayer-settings', settings);
  return response.data;
};

// Get prayer settings
export const getPrayerSettings = async (): Promise<{ success: boolean; data?: PrayerSettings; message?: string }> => {
  try {
