      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <tr key={`skeleton-${rowIndex}`} className="animate-pulse">
          {Array.from({ length: columns }).map((_, colIndex) => (
            <td key={`skeleton-cell-${rowIndex}-${colIndex}`} className="px-4 py-3 md:px-6 md:py-4">
    ...
    <tr>
      <td colSpan={columns} className="px-4 py-8 md:px-6 md:py-12 text-center">
        <div className="flex flex-col items-center justify-center gap-2 md:gap-3">
          ...
          <div className="text-gray-500 font-medium text-base md:text-lg">{message}</div>
          {description && (
            <div className="text-gray-400 text-xs md:text-sm">{description}</div>
          )}
        </div>
      </td>
    </tr>
...
export const Table = <T,>({
  ...
  const containerClasses = [
    responsive && 'overflow-x-auto scrollbar-thin scrollbar-thumb-emerald-400 scrollbar-track-emerald-50',
    'rounded-2xl',
    'border-2',
    'border-emerald-200',
  ...
  return (
    <div className={containerClasses}>
      <div 
        ...
        {...(maxHeight && { style: { ['--max-height' as string]: maxHeight } })}
      >
              ...
              <tr>
                {columns.map((column, index) => (
                  <th
                    ...
                    className={[
                      dense ? 'px-3 py-2 md:px-4 md:py-3 lg:px-6 lg:py-4' : 'px-4 py-3 md:px-6 md:py-4',
                      'font-bold',
                ...
                {renderActions && (
                  <th
                    className={[
                      dense ? 'px-3 py-2 md:px-4 md:py-3' : 'px-4 py-3 md:px-6 md:py-4',
                      'font-bold',
                      'text-sm md:text-base',
                      'text-center',
                      'whitespace-nowrap',
          ...
          <tbody className="divide-y divide-emerald-100/50 bg-white/80">
            {loading ? (
              ...
              data.map((row, rowIndex) => {
                ...
                const rowClasses = [
                  hoverable && 'hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 transition-all duration-300',
                ...
                const rowKey = (rowAny.id || rowAny._id || rowIndex) as string | number;
                ...
                return (
                  ...
                  >
                    ...
                    {renderActions && (
                      <td
                        className={[
                          dense ? 'px-2 py-2 md:px-4 md:py-3' : 'px-3 py-3 md:px-6 md:py-4',
                          'text-center',
                          'align-middle',
                          `w-[${actionsWidth}]`,
                        ]
