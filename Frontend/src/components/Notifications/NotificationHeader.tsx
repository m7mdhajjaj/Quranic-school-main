...
const NotificationHeader: React.FC<NotificationHeaderProps> = ({
  ...
  useEffect(() => {
    ...
    const handleNewNotification = (notification: any) => {
      console.log('📬 إشعار جديد من Socket.IO:', notification);
      
      // ✅ التحقق من وجود sentAt و createdAt
      const sentAtValue = notification.sentAt || new Date().toISOString();
      ...
      console.log('✅ تم إضافة الإشعار للقائمة وتحديث العدادات');
    };

    const handlePrayerNotification = (prayerData: any) => {
      console.log('🕌 إشعار صلاة:', prayerData);
      playNotificationSound();
    };

    const handleQuranReminder = (reminderData: any) => {
      console.log('📖 تذكير قرآن:', reminderData);
    };
