// ============================================================================
// SecretariesList - قائمة السكرتارية
// ============================================================================
...
import type { Secretary } from "@/Api/secretaryApi";
...
export const SecretariesList: React.FC<SecretariesListProps> = ({
  secretaries,
  loading,
  onEdit,
  onDelete,
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center py-20">
        ...
        <Text className="text-gray-600 mt-4">جاري تحميل السكرتارية...</Text>
      </View>
    );
  }
    ...
    return (
      <View className="flex-1 justify-center items-center py-20">
        ...
        <Text className="text-gray-400 text-sm mt-2">
          قم بإضافة سكرتير جديد للبدء
        </Text>
      </View>
    );
  }

  const renderSecretary = ({ item }: { item: Secretary }) => (
    <View className="bg-white rounded-2xl p-4 mb-3 shadow-sm border border-gray-100">
      ...
      <View className="flex-row items-start justify-between mb-3">
        <View className="flex-1">
          ...
          <Text className="text-gray-500 text-xs">#{item.secretaryId}</Text>
        </View>
      </View>
      ...
      <View className="space-y-2">
        ...
        <View className="flex-row gap-2">
          ...
          <View className="flex-1 bg-gray-50 rounded-lg p-2">
            <Text className="text-gray-500 text-xs mb-1">الصلاحيات</Text>
            <Text className="text-gray-800 font-semibold">
              {item.permissions && Object.keys(item.permissions).length > 0
                ? `${Object.keys(item.permissions).length} صلاحية`
                : "لا يوجد"}
            </Text>
          </View>
        </View>
