...
export const DrawerMenu: React.FC<DrawerMenuProps> = ({ isOpen, onClose }) => {
  ...
  const getMenuItems = (): MenuItem[] => {
    ...
    const studentTeacherItems: MenuItem[] = [
      ...
      { to: "/(tabs)/goals", label: "الأهداف", icon: Target },
      { to: "/(tabs)/dailyMarks", label: "العلامات اليومية", icon: Award },
      { to: "/(tabs)/ranking", label: "التصنيف", icon: BarChart },
      { to: "/(tabs)/exam-schedule", label: "جدول الامتحانات", icon: Calendar },
      { to: "/(tabs)/reports", label: "التقارير", icon: FileText },
      { to: "/(tabs)/timetable", label: "المواعيد", icon: Clock },
      { to: "/(tabs)/chat", label: "المحادثة", icon: MessageSquare },
      { to: "/(tabs)/attendance", label: "الحضور والغياب", icon: UserCheck },
