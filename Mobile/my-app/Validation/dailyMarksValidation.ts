...
/**
 * ============================================================================
 * DailyMarks Validation Schema - Mobile
 * ============================================================================
 *
...
export const sectionValidationSchema = yup.object<SectionFormData>({
  date: yup
    ...
    .matches(/^\d{4}-\d{2}-\d{2}$/, "صيغة التاريخ غير صحيحة (YYYY-MM-DD)"),
...
export const markValidationSchema = yup.object<MarkFormData>({
  studentId: yup.string().required("الطالب مطلوب"),

  sectionId: yup.string().required("المقطع مطلوب"),
  ...
  note: yup.string().optional().max(500, "الملاحظة يجب ألا تتجاوز 500 حرف"),
});
 ...
 * Validate mark data
 */
export const validateMark = async (
  data: MarkFormData
): Promise<{
  valid: boolean;
  errors: { [key: string]: string };
