// Teacher Validation using Yup - Mobile
...
export const teacherValidationSchema = yup
  .object({
    ...
    firstName: yup.string().required("الاسم الأول مطلوب").trim(),

    lastName: yup.string().required("اسم العائلة مطلوب").trim(),
    ...
    idNumber: yup
      ...
      .test(
        "exactly-nine-digits",
        "رقم الهوية يجب أن يتكون من 9 أرقام بالضبط",
        function (value) {
          if (!value) return false;
    ...
    gender: yup
      ...
      .nullable()
      .oneOf(
        [null, "ذكر", "أنثى", "male", "female", "Male", "Female"],
    ...
    lastSeen: yup.date().default(() => new Date()),
  })
  .transform((data) => ({
    ...data,
