...
const AZKAR_DATE_KEY = "azkar_date";
const AZKAR_DATA_KEY = "azkar_data";

// البيانات الأصلية للأذكار (مطابقة للـ Frontend)
...
const parseStoredData = (raw: string | null): AzkarCategory[] | null => {
  if (!raw) return null;
  try {
    return JSON.parse(raw) as AzkarCategory[];
  } catch {
    return null;
  }
};
...
export const useAzkar = () => {
  ...
  // تحميل البيانات من AsyncStorage (مطابق لفكرة Frontend - date-based reset)
  useEffect(() => {
    const run = async () => {
      try {
        const todayDate = getTodayDate();
        const savedDate = await StorageHelper.getItem(AZKAR_DATE_KEY);

        if (savedDate !== todayDate) {
          const initial = getInitialAdhkarData();
          setAdhkarData(initial);
          await StorageHelper.setItem(AZKAR_DATE_KEY, todayDate);
          await StorageHelper.setItem(AZKAR_DATA_KEY, JSON.stringify(initial));
        } else {
          const storedRaw = await StorageHelper.getItem(AZKAR_DATA_KEY);
          const stored = parseStoredData(storedRaw);
          if (stored && stored.length > 0) {
            setAdhkarData(stored);
          } else {
            const initial = getInitialAdhkarData();
            setAdhkarData(initial);
        ...
        setAdhkarData(getInitialAdhkarData());
      } finally {
        initializedRef.current = true;
        // نفس إحساس الـ Frontend (skeleton سريع)
  ...
  useEffect(() => {
    ...
    if (!adhkarData || adhkarData.length === 0) return;

    const run = async () => {
      try {
        await StorageHelper.setItem(AZKAR_DATA_KEY, JSON.stringify(adhkarData));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => void checkDateChange(), 60000);
  ...
  const handleDhikrClick = (categoryId: string, dhikrId: number) => {
    setAdhkarData((prev) => {
      return prev.map((category) => {
        ...
        const allCompleted = updatedAdhkar.every((d) => d.count === 0);
        const wasAllCompleted = category.adhkar.every((d) => d.count === 0);

        if (allCompleted && !wasAllCompleted) {
          setTimeout(() => {
            const message = `تم إكمال ${category.title} بنجاح 🎉`;
            if (Platform.OS === "web") {
              window.alert(message);
              setSelectedCategory(null);
              return;
            }
            Alert.alert("مبارك!", message, [
              { text: "الحمد لله", onPress: () => setSelectedCategory(null) },
            ]);
          }, 250);
        }
