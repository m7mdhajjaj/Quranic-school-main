    ...
    try {
      ...
      if (data?.code === 200) {
        ...
        const prayers: PrayerTime[] = [
          { name: "الفجر", time: normalizeTime(timings.Fajr), icon: "🌅" },
          { name: "الشروق", time: normalizeTime(timings.Sunrise), icon: "🌄" },
          { name: "الظهر", time: normalizeTime(timings.Dhuhr), icon: "☀️" },
          { name: "العصر", time: normalizeTime(timings.Asr), icon: "🌤️" },
          { name: "المغرب", time: normalizeTime(timings.Maghrib), icon: "🌆" },
          { name: "العشاء", time: normalizeTime(timings.Isha), icon: "🌙" },
        ];
